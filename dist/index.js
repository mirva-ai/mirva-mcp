...
async function main() {
    if (!credentials.apiKey && !credentials.token)
        usage('MIRVA_API_KEY (or MIRVA_TOKEN) is required');
    const client = new MirvaClient({ url, ...credentials });
    const server = new Server({ name: 'mirva', version }, { capabilities: { tools: { listChanged: true } } });
    ...
    const announceIfChanged = async () => {
        if (!servedDigest || Date.now() - lastDigestCheck < DIGEST_CHECK_INTERVAL_MS)
            return;
    ...
    server.setRequestHandler(ListToolsRequestSchema, async () => {
        ...
        try {
            servedDigest = await client.call('toolsDigest');
        }
        catch {
            servedDigest = undefined;
        }
