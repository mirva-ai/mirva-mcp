 ...
 *
 *   MIRVA_URL=... MIRVA_API_KEY=... node tools/soak.ts <boardId> <x> <y> <w> <h> [rounds] [logFile]
 */
import { appendFileSync } from 'fs';
import { credentialsFromEnv, errorMessage, isImageResult, MirvaClient } from '../src/transport.ts';
import type { Rect } from './board-geometry.ts';
...
const view: Rect = { x: Number(xs), y: Number(ys), w: Number(ws), h: Number(hs) };
...
function regions(): Region[] {
  const out: Region[] = [{ label: 'whole', rect: view, maxDim: 1400 }];
...
async function capture(rect: Rect, maxDim: number): Promise<CaptureResult> {
  const started = performance.now();
  try {
    const r = await client.call('callTool', 'capture_drawing', { shortId: board, rect, maxDim });
    if (isImageResult(r)) return { ms: Math.round(performance.now() - started), ok: true, bytes: Math.round(r.data.length * 0.75) };
    return { ms: Math.round(performance.now() - started), ok: false, bytes: 0, error: 'no image' };
  } catch (e) {
    return { ms: Math.round(performance.now() - started), ok: false, error: errorMessage(e).slice(0, 100) };
  }
}

const startedAt = new Date().toISOString();
const results: (CaptureResult & { round: number; label: string })[] = [];
for (let round = 0; round < rounds; round++) {
  for (const region of regions()) {
    const r = await capture(region.rect, region.maxDim);
    results.push({ round, label: region.label, ...r });
    console.log(`round ${round} ${region.label.padEnd(12)} ${r.ok ? `${r.ms}ms ${Math.round((r.bytes ?? 0) / 1024)}KB` : `FAIL ${r.ms}ms ${r.error}`}`);
...
const summary = {
  ...
  firstRoundMs: Object.fromEntries([...byLabel].map(([l, rs]) => [l, rs[0].ms])),
  warmMedianMs: Object.fromEntries([...byLabel].map(([l, rs]) => {
    const warm = rs.slice(1).filter(r => r.ok).map(r => r.ms).sort((a, b) => a - b);
    return [l, warm.length ? warm[Math.floor(warm.length / 2)] : null];
  })),
