 ...
 *
 * Usage:
 *   MIRVA_URL=... MIRVA_API_KEY=... node tools/call.ts <tool> '<json args>'
 *
 * Options via env:
...
if (!tool) {
  console.error("usage: node tools/call.ts <tool> '<json args>'   (or ARGS_FILE=path)");
  process.exit(2);
}
...
if (source) {
  try {
    ...
    console.error(`arguments are not valid JSON (${where}): ${errorMessage(e)}`);
    process.exit(2);
  }
}
  ...
  if (isImageResult(result)) {
    ...
    if (isTextResult(result)) {
      try {
        ...
        payload = result.text; // plain text, not JSON — hand it back as it came
      }
    }
